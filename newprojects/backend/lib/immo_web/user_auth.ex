defmodule ImmoWeb.UserAuth do
  ...
  def fetch_current_scope_for_user(conn, _opts) do
    with {token, conn} <- ensure_user_token(conn),
      ...
      # query drops a session whose `last_seen_at` is older than 24 h,
      # so a session that has not been touched will be invalidated on
      # the very next request after the timeout window.
  ...
  ## `on_mount` arguments
    ...
    * `:require_admin`, `:require_manager`, `:require_editor`,
      `:require_any_staff` - Layered RBAC per §6.2. The role
      hierarchy is admin > manager > editor; `developer_user` is
      a tenant-bounded role that is NOT in the staff hierarchy and
      must use a separate flow (out of scope for P1-E1.1).
  """
