defmodule Spark.Policy do
  ...
  @tool_risk %{
    # :low
    "read_file" => :low, "list_dir" => :low, "grep" => :low,
    "list_files" => :low, "search" => :low, "head" => :low,
    # :medium
    "write_file" => :medium, "edit_file" => :medium, "web_fetch" => :medium,
    "create_file" => :medium, "replace_in_file" => :medium,
    # :high
    "shell" => :high, "forge_tool" => :high, "exec" => :high,
    "run_command" => :high,
    # :critical
    "rm_rf" => :critical, "destructive_shell" => :critical,
    "secret_access" => :critical, "delete_file" => :critical,
  ...
  def requires_approval?({:tool, tool_name}, _context) do
  ...
  def allowed?({:tool, tool_name}, context), do: allowed?(tool_name, context)
  ...
  """
  def revalidate_plan(%Plan{approval_status: status} = plan) when status in [:draft, :awaiting_approval, :modified] do
    validate_plan(plan)
  end
